...
=end
    ...
    if /[\d._@]/.match(str).present?   # 若包含数字或._, 按照email查找
      ...
      # 先按拼音查找
      # 按首字母
      users = Sys::User.select(select_fields).joins("join sys_user_groups on sys_users.id = sys_user_groups.user_id ").where(["sys_user_groups.group_id in (?) and sys_users.f_letters = ? ",group_ids, "#{str}"]).limit(10).all
      # 按姓
      users = Sys::User.select(select_fields).joins("join sys_user_groups on sys_users.id = sys_user_groups.user_id ").where(["sys_user_groups.group_id in (?) and sys_users.family_name = ? ",group_ids, "#{str}"]).limit(10).all unless users.present?
      # 匹配全拼,连续
      users = Sys::User.select(select_fields).joins("join sys_user_groups on sys_users.id = sys_user_groups.user_id ").where(["sys_user_groups.group_id in (?) and sys_users.pinyin LIKE ? ",group_ids, "%#{str}%"]).limit(10).all unless users.present?
      # 匹配全拼,断续
      regrep_str = ".*" + str.scan(/\w/).join(".*") + ".*"
      users = Sys::User.select(select_fields).joins("join sys_user_groups on sys_users.id = sys_user_groups.user_id ").where(["sys_user_groups.group_id in (?) and sys_users.pinyin REGEXP ? ",group_ids, regrep_str]).limit(10).all unless users.present?
      # 按邮箱查找
      users = Sys::User.select(select_fields).joins("join sys_user_groups on sys_users.id = sys_user_groups.user_id ").where(["sys_user_groups.group_id in (?) and sys_users.email LIKE ? ",group_ids, "%#{str}%"]).limit(10).all unless users.present?
    else  # 按name查找
      users = Sys::User.select(select_fields).joins("join sys_user_groups on sys_users.id = sys_user_groups.user_id ").where(["sys_user_groups.group_id in (?) and sys_users.name LIKE ? ",group_ids, "#{str}%"]).limit(10).all   # 按姓查找
      users = Sys::User.select(select_fields).joins("join sys_user_groups on sys_users.id = sys_user_groups.user_id ").where(["sys_user_groups.group_id in (?) and sys_users.name LIKE ? ",group_ids, "%#{str}%"]).limit(10).all unless users.present?   # 若无该姓，按名查找
    end
    return users
  end
